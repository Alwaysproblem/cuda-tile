// guards
cuda_tile.module @guards_module {
    entry @example_kernel(%A: tile<ptr<f32>>, %B: tile<ptr<f32>>, %size: tile<i32>) {
        %t0 = make_token : token
        ...
        // 逐元素浮点加
        %cond = cmpi less_than %bx, %size, signed : tile<i32> -> tile<i1>
        %0 = constant <f32: 0.0> : !cuda_tile.tile<f32>
        %reshaped_0 = reshape %0 : tile<f32> -> tile<1xf32>
        %boardcasted_0 = broadcast %reshaped_0 : tile<1xf32> -> tile<8xf32>

        // print "Condition: %d\n", %cond : tile<i1>
        // print "size: %d\n", %size : tile<i32>
        %sum = if %cond -> (!cuda_tile.tile<8xf32>) {
            %sum = addf %a, %boardcasted_constant_10 : tile<8xf32>
            yield %sum : tile<8xf32>
        } else {
            %sum = mulf %a, %boardcasted_0 : tile<8xf32>
            yield %sum : tile<8xf32>
        }
