// zip example kernel 
cuda_tile.module @zip_module {
    entry @example_kernel(%A: tile<ptr<f32>>, %B: tile<ptr<f32>>, %C: tile<ptr<f32>>) {
        %t0 = make_token : token
        ...
        %t = make_token : token
        %a, %t_a = load_ptr_tko weak %aptrs_off token=%t : tile<8xptr<f32>> -> tile<8xf32>, token
        %b, %t_b = load_ptr_tko weak %bptrs_off token=%t : tile<8xptr<f32>> -> tile<8xf32>, token

        // output ptr
        %c_ptr_1 = reshape %C : tile<ptr<f32>> -> tile<1xptr<f32>>
        %c_ptr = broadcast %c_ptr_1 : tile<1xptr<f32>> -> tile<8xptr<f32>>

        %cptrs_off = offset %c_ptr, %elem_off
                : tile<8xptr<f32>>, tile<8xi32> -> tile<8xptr<f32>>

        // 逐元素浮点加
        %sum = addf %a, %b : tile<8xf32>

        // print "Data: %f\n", %sum : tile<8xf32>

        // 合并加载得到的两个 token 以确保数据依赖
        // 需等待两个加载都完成后才能写回
        %t_store = join_tokens %t_a, %t_b : token

        // 写回，串到加载得到的 token
        %t3 = store_ptr_tko weak %cptrs_off, %sum token=%t_store
